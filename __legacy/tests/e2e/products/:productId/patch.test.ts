...
const createSeedOfProduct = async () => {
    ...
    const { shop } = await seedDatabaseWithShop(database.client, {}, seededUser.id);
    const { product } = await seedDatabaseWithProduct(database.client, {}, shop.id);
...
describe('END TO END - PATCH /products/:productId', () => {
    test('update an existing product', async () => {
        // Given
        const { authenticatedRequestAgent, product } = await createSeedOfProduct();
    ...
    test('return 401 if not authenticated', async () => {
        ...
        // When
        const response = await requestAgent.patch('/products/' + product.id).send(updatedProduct);
