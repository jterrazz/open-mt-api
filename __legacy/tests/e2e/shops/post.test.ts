...
const { requestAgent, createAuthenticatedRequestAgent, database } = endToEndApplication();
...
describe('END TO END - POST /shops', function () {
    test('creates a new shop', async () => {
        ...
        // When
        const response = await authenticatedRequestAgent.post('/shops').send(params);
    ...
    test('does not create a shop with an handle already existing', async () => {
        // Given
        const { shop: seededShop } = await seedDatabaseWithShop(database.client);
        const { authenticatedRequestAgent } = await createAuthenticatedRequestAgent();
        const params = {
    ...
    test('does not create 2 shops for the same user', async () => {
        ...
        // When
        const firstResponse = await authenticatedRequestAgent.post('/shops').send(params);
        const secondResponse = await authenticatedRequestAgent.post('/shops').send(params);
