...
import { getUserPublicProfileControllerFactory } from '../get-user-public-profile.controller';
...
describe('getUserPublicProfile()', () => {
    test('tracks request', async () => {
        // Given
        const { mockOfCtx, mockOfGetUserPublicProfile, mockOfDeserializer, mockOfSerializer } =
            createMockOfArgs();
        ...
        // Then
        await expect(mockOfCtx.requestTracker.requestedGetUserPublicProfile).toHaveBeenCalledTimes(
            1,
        );
    });
