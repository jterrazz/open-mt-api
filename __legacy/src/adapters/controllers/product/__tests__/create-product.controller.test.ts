...
const createMockOfArguments = () => {
    return {
        mockOfCreateProduct: jest.fn().mockResolvedValue(createMockOfProductEntity()),
...
describe('createProductController()', () => {
    test('calls a tracker event', async () => {
        // Given
        const { mockOfCreateProduct, mockOfShopRepository, mockOfDeserializer, mockOfSerializer } =
        ...
        // Then
        expect(mockOfCtx.requestTracker.requestedCreateProduct).toHaveBeenCalledTimes(1);
    });
