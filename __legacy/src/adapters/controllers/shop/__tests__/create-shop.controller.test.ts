...
import { createShopControllerFactory } from '../create-shop.controller';
...
describe('createShopController()', () => {
    test('calls a tracker event', async () => {
        // Given
        const { mockOfCreateShop, mockOfDeserializer, mockOfSerializer } = createMockOfArgs();
        ...
        // Then
        expect(mockOfCtx.requestTracker.requestedCreateShop).toHaveBeenCalledTimes(1);
    });
