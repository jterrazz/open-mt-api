...
import { createProductFactory } from '../create-product';
...
describe('use-cases / create product', function () {
    it('creates a new product for a shop', async function () {
        // Given
        const { mockOfNewProduct, mockOfProductRepository } = createMockOfArgs();

        // When
        const result = await createProductFactory(mockOfProductRepository)(mockOfNewProduct, 0);
